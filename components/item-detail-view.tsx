...
import { ArrowLeft, Calendar, Plus, ShoppingBag, Check } from "lucide-react"
...
export function ItemDetailView({ id }: { id: string }) {
  ...
  const { getItem, logWear, markSold } = useCloset()
  ...
  if (!item) {
    return (
      <div className="flex min-h-dvh flex-col items-center justify-center gap-4 px-5 text-center">
        <p className="text-sm text-muted-foreground">アイテムが見つかりません</p>
  ...
  const gainLoss = item.salePrice != null ? item.salePrice - item.purchasePrice : null
  const costPerWear =
    item.wearHistory.length > 0 ? Math.round(item.purchasePrice / item.wearHistory.length) : null
    ...
    <div>
      <div className="relative">
        <div className="relative aspect-square w-full bg-muted">
          <Image
            ...
            fill
            sizes="448px"
            className={`object-cover ${sold ? "opacity-70 grayscale" : ""}`}
            priority
          />
        </div>
        <button
      ...
      <div className="px-5 pt-5">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
          <span className="rounded-full bg-secondary px-2.5 py-0.5">{item.category}</span>
          {item.productNumber && (
          ...
          {sold && (
            <span className="rounded-full bg-foreground px-2.5 py-0.5 text-background">売却済み</span>
          )}
        </div>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">
          <Stat label="購入価格" value={formatYen(item.purchasePrice)} />
          <Stat label="購入日" value={formatDateShort(item.purchaseDate)} />
          <Stat label="着用回数" value={`${item.wearHistory.length} 回`} />
          <Stat
            label="1回あたり"
            value={costPerWear != null ? formatYen(costPerWear) : "—"}
          />
        </div>
        ...
        {sold && (
          <div className="mt-3 rounded-xl border border-border bg-card p-4">
            ...
            <div className="mt-2 grid grid-cols-2 gap-3">
              <Stat label="売却価格" value={item.salePrice != null ? formatYen(item.salePrice) : "—"} inset />
              <Stat label="売却日" value={item.saleDate ? formatDateShort(item.saleDate) : "—"} inset />
            </div>
            {gainLoss != null && (
              <p className="mt-3 text-sm">
                損益{" "}
                <span className={gainLoss >= 0 ? "font-medium text-foreground" : "font-medium text-destructive"}>
                  {gainLoss >= 0 ? "+" : "−"}
        ...
        <div className="mt-7">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-medium text-foreground">着用履歴</h2>
            <button
          ...
          {wearOpen && (
            <form
              onSubmit={(e) => {
                e.preventDefault()
                logWear(item.id, { date: wearDate, note: wearNote.trim() || undefined })
                setWearNote("")
                setWearOpen(false)
              }}
            ...
            >
              ...
              >
                <Check className="h-4 w-4" /> 記録する
              </button>
            </form>
          )}

          {item.wearHistory.length === 0 ? (
            <p className="mt-3 text-sm text-muted-foreground">まだ着用履歴がありません</p>
          ) : (
              ...
              {item.wearHistory.map((w, i) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-foreground">{formatDate(w.date)}</p>
                    {w.note && <p className="truncate text-xs text-muted-foreground">{w.note}</p>}
                  </div>
                </li>
              ))}
            </ul>
        ...
        {!sold && (
          <div className="mt-7">
            {!sellOpen ? (
              ...
              >
                <ShoppingBag className="h-4 w-4" /> 売却として記録
              </button>
            ) : (
              <form
                onSubmit={(e) => {
                  e.preventDefault()
                  markSold(item.id, saleDate, Number(salePrice) || 0)
                  setSellOpen(false)
                }}
                className="space-y-3 rounded-xl border border-border bg-card p-4"
              >
                <p className="text-sm font-medium text-foreground">売却情報を入力</p>
                <Field label="売却価格 (¥)">
                  <input
                    inputMode="numeric"
                    value={salePrice}
...
function Stat({ label, value, inset }: { label: string; value: string; inset?: boolean }) {
  return (
    <div className={inset ? "" : "rounded-xl border border-border bg-card p-3.5"}>
